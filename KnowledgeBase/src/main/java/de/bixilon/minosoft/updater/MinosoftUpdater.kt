...
object MinosoftUpdater {
    ...
    // Status: Confirmed Target: validateURL()
    // Purpose: Security check for asset/download URLs
    // Only allows:
    //   - HTTPS (secure, always allowed)
    //   - HTTP only for localhost/127.0.0.1 (dev testing)
    // Throws IllegalArgumentException for insecure remote HTTP
    // Throws IllegalStateException for unsupported protocols (file://, ftp://, etc.)
