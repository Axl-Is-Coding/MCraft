...
import de.bixilon.minosoft.android
...
object Minosoft {
    ...
    private fun postBoot() {
        ...
        if (isAndroid()) {
            // Android UI is already launched, nothing to do here
            Log.log(LogMessageType.GENERAL, LogLevels.INFO) { "Android UI active (QMCL)" }
        } else if (ErosOptions.disabled) {
    ...
    private fun androidUI(taskWorker: TaskTreeBuilder) {
        taskWorker += TreeTask(identifier = BootTasks.ANDROID_UI, executor = {
            AndroidLauncher.launchQMCL()
        })
    }
